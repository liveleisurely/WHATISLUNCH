...
async def recommend_advanced(request: Request):
    try:
        ...
        # 레스토랑 이름 리스트 추출
        restaurant_names = [r[1] for r in all_restaurants]
        ...
        # GPT-3의 응답에서 레스토랑 이름을 데이터베이스의 이름 리스트와 매칭하여 추출
        restaurant_name = next((name for name in restaurant_names if name in recommended_restaurant), None)

        if not restaurant_name:
            raise ValueError("No matching restaurant name found in GPT response.")
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                """
                 ...
                 Please provide a clear answer without repeating the question.
                 If somebody says, 'whatever', then sort randomly and choice.
                 """},
                {"role": "user", "content": full_prompt}
            ]
        )
