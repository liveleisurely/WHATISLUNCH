export const processDailyData = (dailyData, advancedRecommendation = '') => {
  // 상위 5개 항목만 선택
  const top5Data = dailyData.slice(0, 5);

  // 고급 추천 응답에 포함된 레스토랑의 카운트 증가
  top5Data.forEach(item => {
    if (advancedRecommendation.includes(item[0])) {
      item[1] += 1; // 카운트 증가
    }
  });
...
export const processMonthlyData = (monthlyData, advancedRecommendation = '') => {
  ...
  months.forEach((monthName, index) => {
    ...
    top3.forEach(([month, restaurant, count], i) => {
      ...
      // 고급 추천 응답에 포함된 레스토랑의 카운트 증가
      if (advancedRecommendation.includes(restaurant)) {
        count += 1;
      }
...
export const processWeeklyData = (weeklyData, advancedRecommendation = '') => {
...
export const processWeekdayData = (weekdayData, advancedRecommendation = '') => {
